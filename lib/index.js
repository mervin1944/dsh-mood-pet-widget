...
// Whale image: removed in v0.3 — the main display is now a <video> element
// driven by action clips; the static whale PNG (<img> + old /dsh-whale/image.png)
// and its fallback path chain were deleted together.
...
// Sound assets: shipped in assets/ (Ya1/Ya2 duck set, D1/D2 fx1 set).
const SOUND_SETS = {
  duck: {
...
// Action clips (成步堂表情动画) played on whale click.
// v0.3: clips are shipped inside the package (assets/clips/) so the plugin is
// self-contained — no dependency on local absolute paths.
const ACTION_CLIPS_DIR = path.join(PACKAGE_ROOT, 'assets', 'clips')
...
var BALANCE_URL = '/dsh-mood-pet/balance.json'
var SIZE_URL = '/dsh-mood-pet/size.json'
var GIF_URL = '/dsh-mood-pet/rua.gif'
var ACTION_LIST_URL = '/dsh-mood-pet/action/list.json'
var ACTION_DATA_URL = '/dsh-mood-pet/action/data'
var STATIC_CLIP_NAME = '01_站立说话_static.png'
var BUBBLE_BG_URL = '/dsh-mood-pet/assets/bubbles.webp'
// 资源缓存指纹：每次改 assets/clips 下素材时递增，强制浏览器重新拉取（绕过 max-age）
var ASSET_VERSION = 4

var css = [
  '.dshwv-root{position:fixed;right:0;bottom:0;--dshw-scale:1;--dshw-font-scale:1;--dshw-base:clamp(122px,calc(min(250px,min(100vw,100vh) * 0.28) * var(--dshw-scale)),625px);width:var(--dshw-base);height:var(--dshw-base);pointer-events:none;user-select:none;-webkit-user-select:none;z-index:9999;font-family:inherit;transition:left .16s ease,top .16s ease,transform .3s ease}',
  ...
  '.dshwv-bubble{position:absolute;left:0;top:0;width:100%;aspect-ratio:1700/1248;pointer-events:none;z-index:1;--dshw-u:calc(var(--dshw-base) / 1026)}',
  '.dshwv-bubble-bg{position:absolute;left:0;top:0;width:100%;height:100%;display:block;object-fit:contain;pointer-events:none;opacity:0;transform:scale(.85);transform-origin:50% 50%;transition:opacity .2s ease,transform .2s ease;z-index:0}',
  '.dshwv-bubble.dshwv-bubble-open .dshwv-bubble-bg{opacity:1;transform:scale(1)}',
  ...
  '.dshwv-bubble .dshwv-bshape{opacity:0;transform:scale(.7);transform-box:fill-box;transform-origin:50% 50%;transition:opacity .2s ease,transform .2s ease}',
  '.dshwv-bubble.dshwv-bubble-open .dshwv-bshape{opacity:1;transform:none}',
  '.dshwv-gif{position:absolute;left:50%;top:50%;transform:translate(-50%,-50%);max-width:calc(var(--dshw-u) * 560);max-height:calc(var(--dshw-u) * 400);display:none;opacity:0;transition:opacity .2s ease;pointer-events:none;-webkit-user-drag:none;user-select:none;object-fit:contain}',
  '.dshwv-root.dshwv-left .dshwv-gif{transform:translate(-50%,-50%) scaleX(-1)}',
  '.dshwv-bubble.dshwv-bubble-open .dshwv-gif{opacity:1}',
  '.dshwv-bubble.dshwv-bubble-open .dshwv-bshape{transition-delay:.1s}',
  '.dshwv-text{position:absolute;left:50%;top:50%;transform:translate(-50%,-50%) rotate(-12deg);text-align:center;color:#e0433f;line-height:1;white-space:nowrap;pointer-events:none;opacity:0;transition:opacity .16s ease,transform .3s ease;font-family:"AlimamaDongFangDaKai","Arial Black","Impact","Microsoft YaHei",sans-serif;-webkit-text-stroke:calc(var(--dshw-u) * 12) #7a1f18;paint-order:stroke fill;text-shadow:calc(var(--dshw-u) * 5) calc(var(--dshw-u) * 5) 0 rgba(0,0,0,.35);letter-spacing:0}',
  '.dshwv-bubble.dshwv-bubble-open .dshwv-text{opacity:1;transition:opacity .16s ease .36s,transform .3s ease}',
  '.dshwv-root.dshwv-left .dshwv-text{transform:translate(-50%,-50%) rotate(-12deg) scaleX(-1)}',
  '.dshwv-label{font-size:calc(var(--dshw-u) * 62 * var(--dshw-font-scale));font-weight:900;letter-spacing:.02em;color:#e0433f}',
  '.dshwv-amount{font-size:calc(var(--dshw-u) * 140 * var(--dshw-font-scale));font-weight:900;line-height:1;color:#e0433f;-webkit-text-stroke:calc(var(--dshw-u) * 14 * var(--dshw-font-scale)) #7a1f18;}',
  '.dshwv-period{font-size:calc(var(--dshw-u) * 104 * var(--dshw-font-scale));font-weight:900;line-height:1;color:#e0433f}',
  '.dshwv-wrap{white-space:normal;max-width:calc(var(--dshw-u) * 560 * var(--dshw-font-scale));line-height:1.1}',
  '.dshwv-hint{font-size:calc(var(--dshw-u) * 56 * var(--dshw-font-scale));color:#c1443c;letter-spacing:.02em;margin-top:calc(var(--dshw-u) * 9 * var(--dshw-font-scale));min-height:calc(var(--dshw-u) * 64);line-height:1.15}',
  ...
  '@font-face{font-family:"AlimamaDongFangDaKai";src:url("/dsh-mood-pet/fonts/AlimamaDongFangDaKai.ttf?v=' + ASSET_VERSION + '") format("truetype");font-weight:normal;font-style:normal;font-display:swap;}'
].join('\\n')
...
// 字体大小缩放（气泡文字）
var FONT_SCALE_MIN = 0.5
var FONT_SCALE_MAX = 2.0
var fontScaleInput = document.createElement('input')
fontScaleInput.type = 'range'
fontScaleInput.min = String(FONT_SCALE_MIN)
fontScaleInput.max = String(FONT_SCALE_MAX)
fontScaleInput.step = '0.05'
fontScaleInput.className = 'dshwv-range'
fontScaleInput.value = '1'
var fontScaleNumber = document.createElement('input')
fontScaleNumber.type = 'number'
fontScaleNumber.min = '50'
fontScaleNumber.max = '200'
fontScaleNumber.step = '5'
fontScaleNumber.className = 'dshwv-number'
fontScaleNumber.value = '100'
fontScaleInput.addEventListener('input', function () { setFontScale(fontScaleInput.value) })
fontScaleNumber.addEventListener('input', function () {
  var v = Math.round(Number(fontScaleNumber.value))
  setFontScale(Math.max(FONT_SCALE_MIN, Math.min(FONT_SCALE_MAX, v / 100)))
})
fontScaleNumber.addEventListener('change', function () {
  var v = Math.round(Number(fontScaleNumber.value))
  setFontScale(Math.max(FONT_SCALE_MIN, Math.min(FONT_SCALE_MAX, v / 100)))
})
...
var rowFont = menuRow()
rowFont.appendChild(menuLabel('字体'))
rowFont.appendChild(fontScaleInput)
rowFont.appendChild(fontScaleNumber)
var row2 = menuRow()
...
// 逆转裁判"异议!"气泡：用真实的逆转裁判气泡背景图（从 ObjectionBubbleGenerator
// 素材库下载的 bubbles.webp），叠加红色阿里妈妈冲击字。
var bubbleBgImg = document.createElement('img')
bubbleBgImg.className = 'dshwv-bubble-bg'
bubbleBgImg.alt = ''
bubbleBgImg.src = BUBBLE_BG_URL + '?v=' + ASSET_VERSION
bubbleBgImg.draggable = false
bubbleBox.appendChild(bubbleBgImg)
...
// —— 主显示：全量预加载 <img> GIF 池（v0.3.2）——
// 视频叠放合成不稳定导致切换闪烁，改用 GIF。GIF 自带循环，无需
// ended/重播/seek 逻辑。启动时把每个 clip 各建一个 <img> 并预载，
// 切换情绪只是从池里选目标 img 做显隐——无解码空窗，零闪烁。
// 说明：GIF 无法"暂停在首帧"，idle 静止用一张 PNG 静帧（mainStatic）展示；
//       其余情绪播对应 GIF（循环）。
var MAIN_CSS = 'position:absolute;left:0;top:0;width:100%;height:100%;display:none;object-fit:contain;object-position:bottom center;pointer-events:none;z-index:3;transform:scaleX(-1)'
function makeMainImg() {
  var img = document.createElement('img')
  img.className = 'dshwv-main-img'
  img.alt = ''
  img.draggable = false
  img.style.cssText = MAIN_CSS
  img.addEventListener('error', function () { showStaticClip() })
  return img
}
// preloaded pool: name -> {img, ready}
var imgPool = {}
var curImg = null // 当前可见的 img
var curClipName = null // 当前显示的剪辑文件名

function clipNameUrl(name) {
  // 追加缓存指纹版本号：资源内容变更时递增 ASSET_VERSION，强制浏览器重新请求
  return ACTION_DATA_URL + '?name=' + encodeURIComponent(name) + '&v=' + ASSET_VERSION
}
// 预载一个 clip 到池里（不打断当前显示）。mainStatic 用 PNG 静帧，其余用 GIF。
function ensureClipInPool(name, isStaticFrame) {
  if (imgPool[name]) return imgPool[name]
  var img = makeMainImg()
  img.draggable = false
  img.src = clipNameUrl(name)
  body.appendChild(img)
  var item = { img: img, ready: false }
  imgPool[name] = item
  img.addEventListener('load', function () { item.ready = true }, { once: true })
  return item
}
// 播放指定 clip：从池取目标 img，显隐切换（无动画，纯硬切）。
// isStaticFrame=true 表示静态帧（idle 待机）；isSingle=true 表示单次播放，
// 播放完按 duration 定时回静态（GIF/WebP 的 <img> 无 ended 事件，需定时器）。
var actionReturnTimer = null
function clearActionReturn() {
  if (actionReturnTimer) { clearTimeout(actionReturnTimer); actionReturnTimer = null }
}
// 各剪辑时长（秒，用于单次播放后自动回静态）。0 = 循环/静态不自动回。
var CLIP_DURATION = {
  '01_站立说话.webp': 3.0,
  '02_拍桌喊话.webp': 3.3,
  '03_手指前指控诉.webp': 2.9,
...
function playClipFile(name, isStaticFrame, isSingle) {
  if (!name) return
  clearActionReturn()
  if (curClipName === name && curImg) return // 已在播放
  var item = ensureClipInPool(name, isStaticFrame)
  var next = item.img
  if (curImg && curImg !== next) {
    curImg.style.display = 'none'
  }
  next.style.display = 'block'
  curImg = next
  curClipName = name
  // 单次播放：播放到达时长后自动回静态帧（覆盖当前情绪的动作/单次情绪）
  if (isSingle) {
    var dur = CLIP_DURATION[name] || 3
    actionReturnTimer = setTimeout(function () {
      actionReturnTimer = null
      if (curClipName === name) showStaticClip()
    }, dur * 1000)
  }
}

var actionClips = []
// 情绪 → 剪辑映射（透明动画 WebP 文件名；idle 用 PNG 静帧）
var MOOD_CLIPS = {
  thinking: ['05_托腮思考一.webp', '07_托腮思考二.webp'],
  reading:  ['06_举纸展示一.webp', '08_举纸展示二.webp'],
  writing:  ['03_手指前指控诉.webp'],
  searching: ['09_特写反应镜头.webp'],
  running:  ['04_双手扶腰说话.webp'],
  config:   ['02_拍桌喊话.webp'],
  speaking: ['01_站立说话.webp'],
}
var MOOD_SINGLE = { writing: 1, searching: 1, running: 1, config: 1 } // 只播一次不循环
var moodNow = 'idle'
function showStaticClip() {
  // 静止帧 = 站立说话 PNG 静帧（不循环、不动）
  playClipFile(STATIC_CLIP_NAME, true)
}
function pickClip(list) {
  return list[Math.floor(Math.random() * list.length)]
}
function playMoodClip() {
  if (!moodNow || moodNow === 'idle' || !MOOD_CLIPS[moodNow]) { showStaticClip(); return }
  // MOOD_SINGLE 的情绪（写/搜/跑/配）只播一次，播完回静态；其余循环
  playClipFile(pickClip(MOOD_CLIPS[moodNow]), false, !!MOOD_SINGLE[moodNow])
}
// 手动点击：播放随机动作，覆盖当前情绪，播完回静态
function playActionClip() {
  if (actionClips.length === 0) return
  var clip = actionClips[Math.floor(Math.random() * actionClips.length)]
  playClipFile(clip.name, false, true)
}
// 情绪轮询
var MOOD_URL = '/dsh-mood-pet/mood.json'
function pollMood() {
  fetch(MOOD_URL, { cache: 'no-store' }).then(function (r) { return r.json() }).then(function (d) {
    if (!d || !d.ok || typeof d.mood !== 'string') return
    if (d.mood === moodNow) return
    moodNow = d.mood
    if (moodNow === 'idle') { showStaticClip(); return }
    playMoodClip()
  }).catch(function () {})
}
// 初始化：预载全部剪辑 + 显示静态帧，再取剪辑列表与首次情绪
setTimeout(function () {
  // 预载所有用到的 GIF + 静态 PNG 帧
  var names = [STATIC_CLIP_NAME]
  Object.keys(MOOD_CLIPS).forEach(function (k) {
    (MOOD_CLIPS[k] || []).forEach(function (n) { names.push(n) })
  })
  names = names.filter(function (n, i) { return names.indexOf(n) === i })
  names.forEach(function (n) { ensureClipInPool(n) })
  showStaticClip()
  fetch(ACTION_LIST_URL, { cache: 'no-store' }).then(function (r) { return r.json() }).then(function (d) {
    if (d && d.ok && Array.isArray(d.clips)) actionClips = d.clips
  }).catch(function () {})
  fetch(MOOD_URL, { cache: 'no-store' }).then(function (r) { return r.json() }).then(function (d) {
    if (d && d.ok && d.mood && d.mood !== 'idle') { moodNow = d.mood; playMoodClip() }
  }).catch(function () {})
}, 100)
setInterval(pollMood, 1500)
...
// 统一气泡状态机：'hidden' | 'balance' | 'random' | 'cost'
// 优先级 cost > balance > random。避免三种内容互相抢占同一气泡框。
var bubbleMode = 'hidden'
...
function showBubble(mode) {
  if (!bubbleOn) return
  var want = mode || 'balance'
  // 消耗金额泡泡进行中：余额/随机气泡不打断（cost 优先级最高）
  if (costBubbleActive) return
  // 随机打开时，余额变动不覆盖正在看的台词（除非是用户点击主动重置）
  if (bubbleMode === 'random' && want !== 'random' && want !== 'force') return
  if (bubbleTimer) { clearTimeout(bubbleTimer); bubbleTimer = null }
  if (gifFadeTimer) { clearTimeout(gifFadeTimer); gifFadeTimer = null }
  bubbleShown = true
...
function saveConfig() {
  try {
    fetch(SIZE_URL, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ scale: state.scale, fontScale: state.fontScale, sound: soundOn, vol: soundVol, soundSet: soundSet, usageMode: usageMode, peakMode: peakMode, bubbleOn: bubbleOn, turnCostOn: turnCostOn, turnCostCloseMs: turnCostCloseMs, scrollGapOn: scrollGapOn, scrollGapPx: scrollGapPx }) })
...
function setFontScale(v) {
  var next = Math.round(Math.min(FONT_SCALE_MAX, Math.max(FONT_SCALE_MIN, Number(v))) * 100) / 100
  state.fontScale = next
  root.style.setProperty('--dshw-font-scale', String(next))
  fontScaleInput.value = String(next)
  fontScaleNumber.value = String(Math.round(next * 100))
  saveConfig()
}
function setVol(v) {
...
function applySoundSet() {
  try {
    pressAudio = new Audio('/dsh-mood-pet/sound/press.mp3?set=' + soundSet)
    pressAudio.preload = 'auto'
    pressAudio.volume = soundVol
    releaseAudio = new Audio('/dsh-mood-pet/sound/release.mp3?set=' + soundSet)
...
function isWhaleHit(e) {
  // 视频覆盖整个挂件区域，只要点落在 root 内即视为点击鲸鱼
  try {
    var r = root.getBoundingClientRect()
...
function onDocClickStopper(e) {
  // 菜单/菜单按钮/气泡区域内的点击永远放行——命中检测是纯坐标+像素透明度
  // 判断，不知道上层还盖着菜单 UI；若几何上与鲸鱼图不透明区域重叠（例如
  // 汉堡按钮天然贴在图片右上角），不加这层排除会导致点击被吞、菜单打不开。
  if (e.target && e.target.closest && (e.target.closest('.dshwv-bubble') || e.target.closest('.dshwv-menu') || e.target.closest('.dshwv-menu-btn'))) return
...
function endDrag(e, clickAllowed) {
  ...
  if (clickAllowed && !drag.moved) { playActionClip(); showBubble('force'); refresh(true); return }
  var dx = e.clientX - drag.startX
  var dy = e.clientY - drag.startY
...
fetch(SIZE_URL, { cache: 'no-store' })
  ...
  .then(function (d) {
    ...
    if (d && typeof d.fontScale === 'number' && d.fontScale >= FONT_SCALE_MIN - 0.1 && d.fontScale <= FONT_SCALE_MAX + 0.1) {
      state.fontScale = d.fontScale
      root.style.setProperty('--dshw-font-scale', String(d.fontScale))
      fontScaleInput.value = String(d.fontScale)
      fontScaleNumber.value = String(Math.round(d.fontScale * 100))
    }
...
var LAST_TURN_URL = '/dsh-mood-pet/last-turn.json'
var lastCostSeq = 0
...
const name = 'dsh-mood-pet-widget'
...
function apply(ctx) {
    ...
    // —— 智能情绪追踪：根据 session 事件推断 AI 当前状态 ——
    let currentMood = 'idle'
    let moodSetAt = 0
    let moodTimer = null
    // 情绪历史（调试/验证用）：记录最近一次成功切换的来源，绕开"查询工具
    // 本身会污染当前情绪"的干扰——看历史即可确认每个工具触发过什么情绪。
    let moodHistory = []
    const MOOD_HISTORY_MAX = 30
    const MOOD_IDLE_MS = 25000
    const MOOD_TOOL_MAP = {
      read: 'reading', read_image: 'reading', glob: 'reading', grep: 'reading',
      list_agents: 'reading',
      edit: 'writing', write: 'writing', todo_write: 'writing',
      web_search: 'searching',
      subagent: 'thinking', subagent_fork: 'thinking', workflow: 'thinking',
      ralph: 'thinking', skill: 'thinking', send_message: 'thinking',
      interrupt_agent: 'thinking', ask_user_question: 'thinking',
      pwsh: 'running', bash: 'running',
      job_output: 'running', job_list: 'running', job_kill: 'running',
      plugin_manage: 'config', plugin_discover: 'config',
      config_backup: 'config', config_restore: 'config',
      config_list_snapshots: 'config', config_sync_pull: 'config',
      config_sync_push: 'config', create_goal: 'config', get_goal: 'config',
      update_goal: 'config', learn_meme: 'config', send_meme: 'config',
    }
    function moodForTool(name) {
      return MOOD_TOOL_MAP[String(name || '').toLowerCase()] || 'thinking'
    }
    function setMood(mood, source) {
      const now = Date.now()
      const src = source || 'unknown'
      // 3 秒最短展示保护：非 idle → 非 idle 且距上次切换 <3s 时拒绝覆盖。
      // idle 不受此限（闲置状态优先级最高）。
      const isProtected = mood !== 'idle' && currentMood !== 'idle' && now - moodSetAt < 3000
      const applied = mood !== currentMood && !isProtected
      // 历史记录每次尝试（含被拒），applied 标注是否真正生效——
      // 便于验证各工具触发的情绪，以及观察保护机制是否误伤。
      moodHistory.push({ mood, source: src, ts: now, applied })
      ...
      if (mood !== 'idle') {
        moodTimer = setTimeout(() => { currentMood = 'idle' }, MOOD_IDLE_MS)
      }
    }
    ...
    function handleSessionEvent(sessionId, event) {
      try {
        ...
        // === 情绪追踪：在所有事件类型上都跑，抢在早期 return 前 ===
        if (type === 'step/start') {
          setMood('thinking', 'step/start')
        } else if (type === 'tool/call') {
          const toolName = d && d.name
          setMood(moodForTool(toolName), toolName ? 'tool:' + toolName : 'tool/call')
        } else if (type === 'tool/result') {
          setMood('thinking', 'tool/result')
        } else if (type === 'assistant/message') {
          setMood('speaking', 'assistant/message')
        } else if (type === 'turn/end') {
          setMood('idle', 'turn/end')
        }
    ...
    // 动作表情剪辑：读取包内 assets/clips 目录下的 GIF 列表（手动点击随机动作用）
    let actionClipCache = null
    function listActionClips() {
      if (actionClipCache) return actionClipCache
      try {
        const files = fs.readdirSync(ACTION_CLIPS_DIR).filter((f) => f.endsWith('.webp')).sort()
        actionClipCache = files.map((f) => ({
          name: f,
          url: '/dsh-mood-pet/action/' + encodeURIComponent(f),
        }))
        return actionClipCache
      } catch (err) {
        return []
      }
    }
    ...
    function readActionClip(filename) {
      const safe = path.basename(String(filename || ''))
      // 允许 .webp / .gif / .png / .webm
      if (!/\.(webp|gif|png|webm)$/i.test(safe)) return null
      const full = path.join(ACTION_CLIPS_DIR, safe)
      try {
    ...
    function writeSizeConfig(scale, fontScale, sound, vol, soundSet, usageMode, peakMode, bubbleOn, turnCostOn, turnCostCloseMs, scrollGapOn, scrollGapPx) {
    ...
    disposers.push(ctx.webServer.register({
      kind: 'exact',
      path: '/dsh-mood-pet/fonts/AlimamaDongFangDaKai.ttf',
      handler: (req, res) => {
        try {
          const bytes = fs.readFileSync(path.join(PACKAGE_ROOT, 'assets', 'fonts', 'AlimamaDongFangDaKai.ttf'))
          res.writeHead(200, {
          ...
          res.writeHead(404, { 'Content-Type': 'text/plain; charset=utf-8' })
          res.end('font unavailable: ' + String((err && err.message) || err))
        }
      },
    }))
    ...
    disposers.push(ctx.webServer.register({
      kind: 'exact',
      path: '/dsh-mood-pet/assets/bubbles.webp',
      handler: (req, res) => {
        try {
          const bytes = fs.readFileSync(path.join(PACKAGE_ROOT, 'assets', 'bubbles.webp'))
          res.writeHead(200, {
          ...
          res.writeHead(404, { 'Content-Type': 'text/plain; charset=utf-8' })
          res.end('bubble bg unavailable: ' + String((err && err.message) || err))
        }
      },
    }))
    ...
    disposers.push(ctx.webServer.register({
      kind: 'exact',
      path: '/dsh-mood-pet/action/list.json',
      handler: (req, res) => {
        const clips = listActionClips()
        res.writeHead(200, JSON_HEADERS)
        res.end(JSON.stringify({ ok: true, clips }))
      },
    }))

    disposers.push(ctx.webServer.register({
      kind: 'exact',
      path: '/dsh-mood-pet/action/data',
      handler: (req, res) => {
        try {
          // query: ?name=01_站立说话.gif | image .png | .webm
          const q = String(req.url || '').split('?')[1] || ''
          const m = /(?:^|&)name=([^&]+)/.exec(q)
          const filename = m ? decodeURIComponent(m[1]) : ''
          const bytes = readActionClip(filename)
          if (bytes && bytes.length > 0) {
            res.writeHead(200, {
              'Content-Type': contentTypeFor(filename),
              // 静态素材（GIF/PNG/WebM）名称不变即内容不变，可长期缓存
    ...
    // 智能情绪状态端点：返回当前 AI 操作状态，供前端切换表情动画
    disposers.push(ctx.webServer.register({
      kind: 'exact',
      path: '/dsh-mood-pet/mood.json',
      handler: (req, res) => {
        res.writeHead(200, JSON_HEADERS)
        res.end(JSON.stringify({ ok: true, mood: currentMood, history: moodHistory }))
      },
    }))

    disposers.push(ctx.webServer.register({
      kind: 'exact',
      path: '/dsh-mood-pet/balance.json',
    ...
    disposers.push(ctx.webServer.register({
      kind: 'exact',
      path: '/dsh-mood-pet/last-turn.json',
      handler: (req, res) => {
      ...
      handler: async (req, res) => {
        if (req.method === 'PUT' || req.method === 'POST') {
          try {
            ...
            const result = writeSizeConfig(scale, parsed.fontScale, parsed.sound !== false, parsed.vol, parsed.soundSet, parsed.usageMode, parsed.peakMode, parsed.bubbleOn, parsed.turnCostOn, parsed.turnCostCloseMs, parsed.scrollGapOn, parsed.scrollGapPx)
    ...
    disposers.push(ctx.webServer.register({
      kind: 'exact',
      path: '/dsh-mood-pet/sound/press.mp3',
      handler: (req, res) => {
    ...
    disposers.push(ctx.webServer.register({
      kind: 'exact',
      path: '/dsh-mood-pet/sound/release.mp3',
      handler: (req, res) => {
        const set = SOUND_SETS[soundSetFromUrl(req.url)] || SOUND_SETS.duck
    ...
    disposers.push(ctx.webServer.tapIndex((html) => {
      if (html.indexOf('/dsh-mood-pet/widget.js') !== -1) return html
      const tag = '<script defer src="/dsh-mood-pet/widget.js"></script>'
