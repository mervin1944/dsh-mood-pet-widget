...
// 主显示元素：视频替代静态图，默认展示 01_站立说话.webm 第一帧。
// 前端持续轮询 /dsh-mood-pet/mood.json，根据 AI 当前操作状态自动切换表情。
var mainVid = document.createElement('video')
mainVid.className = 'dshwv-main-vid'
mainVid.muted = true
mainVid.playsInline = true
mainVid.loop = false
mainVid.preload = 'auto'
mainVid.style.cssText = 'position:absolute;left:0;top:0;width:100%;height:100%;display:block;object-fit:contain;object-position:bottom center;pointer-events:none;z-index:3;transform:scaleX(-1)'
mainVid.setAttribute('playsinline', '')
mainVid.setAttribute('webkit-playsinline', '')
body.appendChild(mainVid)
var actionClips = []
// 情绪 → 剪辑映射
var MOOD_CLIPS = {
...
function showStaticClip() {
  if (mainVid.src.indexOf(encodeURIComponent(STATIC_CLIP_NAME)) !== -1) return
  mainVid.src = ACTION_DATA_URL + '?name=' + encodeURIComponent(STATIC_CLIP_NAME)
  mainVid.load()
}
...
function playMoodClip() {
  ...
  var clip = pickClip(MOOD_CLIPS[moodNow])
  mainVid.src = ACTION_DATA_URL + '?name=' + encodeURIComponent(clip)
  try { mainVid.load(); mainVid.play().catch(function () {}) } catch (err) {}
}
// 视频播完后的行为：单次情绪 clip 恢复静态；循环情绪 clip 重播；idle 恢复静态
mainVid.addEventListener('ended', function () {
  if (moodNow === 'idle' || !moodNow) { showStaticClip(); return }
  if (MOOD_SINGLE[moodNow]) { showStaticClip(); return }
  // thinking/reading/speaking 循环播放
  playMoodClip()
})
mainVid.addEventListener('error', function () { showStaticClip() })
// 手动点击：播放随机动作，覆盖当前情绪
function playActionClip() {
  if (actionClips.length === 0) return
  var clip = actionClips[Math.floor(Math.random() * actionClips.length)]
  mainVid.src = ACTION_DATA_URL + '?name=' + encodeURIComponent(clip.name)
  try { mainVid.load(); mainVid.play().catch(function () {}) } catch (err) {}
}
// 情绪轮询
...
function apply(ctx) {
    ...
    function setMood(mood, source) {
      // idle 只记录不重复（避免超时把历史刷满 idle）；同一情绪且未到展示期不记
      if (currentMood === mood && mood === 'idle') return
      if (currentMood === mood) return
      // 每个情绪至少展示 3 秒，避免快速连续工具调用（如 read → pwsh 查状态）
      // 把上一个情绪冲掉来不及展示。idle 不受此限（闲置状态优先级最高）。
      if (mood !== 'idle' && currentMood !== 'idle' && Date.now() - moodSetAt < 3000) return
      currentMood = mood
      moodSetAt = Date.now()
      moodHistory.push({ mood, source: source || 'unknown', ts: Date.now() })
