...
// —— 主显示：双缓冲 video（v0.3.1）——
// 之前单 <video> 每次换源（情绪切换/循环重播）都 load() 新 webm，
// 解码空窗导致画面"闪一下"。现在用两个叠放的 video：
//   新 clip 先在后台 video 预载解码 → canplay 后再淡入淡出切换；
//   同一 clip 重播直接 currentTime=0，不换源 → 循环零闪烁。
...
var vidA = makeMainVideo()
var vidB = makeMainVideo()
vidA.style.opacity = '1'
vidB.style.opacity = '0'
body.appendChild(vidA)
body.appendChild(vidB)
var curVid = vidA // 当前可见的 video
var curClipName = null // 当前显示的剪辑文件名（null = 尚未就绪）
var switchSeq = 0 // 切换序号：丢弃过期回调，防止快速切源竞态

function clipNameUrl(name) {
  return ACTION_DATA_URL + '?name=' + encodeURIComponent(name)
}
// 把 name 加载进后台 video，canplay 后淡入淡出切换；shouldPlay=false 时停首帧（静态）。
function playClipFile(name, shouldPlay) {
  ...
  var seq = ++switchSeq
  var next = (curVid === vidA) ? vidB : vidA
  next.src = clipNameUrl(name)
  next.load()
  var done = false
  function commit() {
    if (done || seq !== switchSeq) return
    done = true
    if (curVid !== next) {
  ...
  // canplay/loadeddata 任一就绪即切换；2.5s 兜底防止解码失败卡死
