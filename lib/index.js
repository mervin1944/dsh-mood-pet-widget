...
var ACTION_DATA_URL = '/dsh-mood-pet/action/data.webm'
var STATIC_CLIP_NAME = '01_站立说话.webm'
...
// —— 主显示：全量预加载 video 池（v0.3.1）——
// 之前单 <video> 每次换源都 load() 新 webm，解码空窗导致"闪一下"。
// 改为：启动时把每个 clip 各建一个 video 并全部预解码（loadeddata），
// 切换情绪时只是从池里选目标 video 做淡入淡出 + play——不再当次加载，
// 彻底消除切换/循环的加载空窗。同 clip 循环用 currentTime=0 无缝重播。
var VID_CSS = 'position:absolute;left:0;top:0;width:100%;height:100%;display:block;object-fit:contain;object-position:bottom center;pointer-events:none;z-index:3;transform:scaleX(-1);transition:opacity .14s ease'
function makeMainVideo() {
  var v = document.createElement('video')
  v.className = 'dshwv-main-vid'
  v.muted = true
  v.playsInline = true
  v.loop = false
  v.preload = 'auto'
  ...
  v.addEventListener('ended', onMainVidEnded)
  v.addEventListener('error', function () { showStaticClip() })
  return v
}
// preloaded pool: name -> {vid, ready}
var vidPool = {}
var poolReady = false
var curVid = null // 当前可见的 video
var curClipName = null // 当前显示的剪辑文件名
var switchSeq = 0 // 切换序号：丢弃过期回调，防止快速切源竞态

function clipNameUrl(name) {
  return ACTION_DATA_URL + '?name=' + encodeURIComponent(name)
}
// 预载一个 clip 到池里（不打断当前显示）。shouldPreload=true 时建 video 并解码。
function ensureClipInPool(name) {
  if (vidPool[name]) return vidPool[name]
  var v = makeMainVideo()
  v.style.opacity = '0'
  v.src = clipNameUrl(name)
  v.load()
  body.appendChild(v)
  var item = { vid: v, ready: false }
  vidPool[name] = item
  v.addEventListener('loadeddata', function () { item.ready = true }, { once: true })
  v.addEventListener('canplay', function () { item.ready = true }, { once: true })
  return item
}
// 播放指定 clip：从池取目标 video，淡入淡出切换；shouldPlay=false 时停首帧（静态）。
function playClipFile(name, shouldPlay) {
  if (!name) return
  if (curClipName === name && curVid) {
    // 同一 clip：不换源，无缝重播（循环场景的关键——零闪烁）
    try {
      if (shouldPlay) { curVid.currentTime = 0; curVid.play().catch(function () {}) }
      else { curVid.pause(); curVid.currentTime = 0 }
    } catch (err) {}
    return
  }
  var item = ensureClipInPool(name)
  var next = item.vid
  var seq = ++switchSeq
  function commit() {
    if (seq !== switchSeq) return
    if (curVid && curVid !== next) {
      curVid.style.opacity = '0'
      try { curVid.pause() } catch (err) {}
    }
    next.style.opacity = '1'
    curVid = next
    curClipName = name
    try {
      if (shouldPlay) { next.currentTime = 0; next.play().catch(function () {}) }
      else { next.pause(); next.currentTime = 0 }
    } catch (err) {}
  }
  if (item.ready) {
    // 已解码：立即切换，零延迟
    ...
    // 尚未解码完成：等 loadeddata/canplay 或兜底再切
    next.addEventListener('loadeddata', commit, { once: true })
    next.addEventListener('canplay', commit, { once: true })
    setTimeout(function () { if (seq === switchSeq) commit() }, 2500)
  }
}
// 启动时预载所有用到的剪辑，避免首个情绪切换时首次加载卡顿
function preloadAllClips() {
  ...
  // 初始显示：站立说话第一帧（静态）
  poolReady = true
  playClipFile(STATIC_CLIP_NAME, false)
}

var actionClips = []
// 情绪 → 剪辑映射
var MOOD_CLIPS = {
  thinking: ['05_托腮思考一.webm', '07_托腮思考二.webm'],
  reading:  ['06_举纸展示一.webm', '08_举纸展示二.webm'],
  writing:  ['03_手指前指控诉.webm'],
  searching: ['09_特写反应镜头.webm'],
  running:  ['04_双手扶腰说话.webm'],
  config:   ['02_拍桌喊话.webm'],
  speaking: ['01_站立说话.webm'],
}
...
function playMoodClip() {
  ...
  playClipFile(pickClip(MOOD_CLIPS[moodNow]), true)
}
// 视频播完：单次情绪回静态；循环情绪重播（同 clip 无缝续播/异 clip 双缓冲切换）
function onMainVidEnded() {
  if (this !== curVid) return // 只处理当前可见 video 的 ended
  if (moodNow === 'idle' || !moodNow) { showStaticClip(); return }
  if (MOOD_SINGLE[moodNow]) { showStaticClip(); return }
  // thinking/reading/speaking 循环播放
  playMoodClip()
}
// 手动点击：播放随机动作，覆盖当前情绪
...
// 初始化：启动即预载全部剪辑 + 显示站立静止帧，再取剪辑列表与首次情绪
setTimeout(function () {
  preloadAllClips()
...
function apply(ctx) {
    ...
    // 动作表情剪辑：读取包内 assets/clips 目录下的 WebM 文件列表
    let actionClipCache = null
    function listActionClips() {
      if (actionClipCache) return actionClipCache
      try {
        const files = fs.readdirSync(ACTION_CLIPS_DIR).filter((f) => f.endsWith('.webm')).sort()
    ...
    function readActionClip(filename) {
      ...
      if (!safe.endsWith('.webm')) return null
    ...
    disposers.push(ctx.webServer.register({
      kind: 'exact',
      path: '/dsh-mood-pet/action/data.webm',
      handler: (req, res) => {
        try {
          // query: ?name=01_站立说话.webm
