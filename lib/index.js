...
var css = [
  ...
  // —— 逆转裁判风格冲击大字（异议!/待った!/くらえ!）——
  '.dshwv-impact{position:absolute;left:50%;top:50%;transform:translate(-50%,-50%);z-index:6;pointer-events:none;opacity:0;visibility:hidden;--dshw-u:calc(var(--dshw-base) / 1026)}',
  '.dshwv-impact.dshwv-impact-show{opacity:1;visibility:visible;animation:dshwv-impact-in .18s cubic-bezier(.34,1.56,.64,1) forwards,dshwv-impact-out .3s ease .9s forwards}',
  '@keyframes dshwv-impact-in{0%{transform:translate(-50%,-50%) scale(.3) rotate(-8deg);opacity:0}60%{transform:translate(-50%,-50%) scale(1.15) rotate(2deg);opacity:1}100%{transform:translate(-50%,-50%) scale(1) rotate(0);opacity:1}}',
  '@keyframes dshwv-impact-out{0%{transform:translate(-50%,-50%) scale(1);opacity:1}100%{transform:translate(-50%,-50%) scale(1.6);opacity:0}}',
  '.dshwv-impact .dshwv-impact-text{font-weight:900;font-size:calc(var(--dshw-u) * 210);line-height:1;white-space:nowrap;color:#fff;-webkit-text-stroke:calc(var(--dshw-u) * 14) #1a1a2e;paint-order:stroke fill;text-shadow:0 calc(var(--dshw-u) * 10) 0 rgba(0,0,0,.25);letter-spacing:.02em}',
  '.dshwv-impact .dshwv-impact-text.dshwv-impact-red{color:#e84a3a}',
  '.dshwv-impact .dshwv-impact-deco{position:absolute;left:50%;top:50%;transform:translate(-50%,-50%);width:130%;height:110%;z-index:-1;pointer-events:none}'
].join('\\n')
...
// —— 逆转裁判风格冲击大字：情绪 → 文案映射 ——
var SHOUT_MAP = {
  config:   { t: '异议!', red: true },   // 拍桌喊话 → 异议！
  writing:  { t: '待った!', red: false }, // 指控/写代码 → 待った！
  searching:{ t: 'くらえ!', red: false }, // 特写/搜索 → くらえ！
  running:  { t: 'キタ!', red: false },  // 运行命令 → キタ！
}
var impactEl = null
var impactTextEl = null
function ensureImpact() {
  if (impactEl) return
  impactEl = document.createElement('div')
  impactEl.className = 'dshwv-impact'
  impactEl.innerHTML = '<div class="dshwv-impact-deco"></div><div class="dshwv-impact-text"></div>'
  impactTextEl = impactEl.querySelector('.dshwv-impact-text')
  body.appendChild(impactEl)
}
var shoutTimer = null
function showShout(mood) {
  var s = SHOUT_MAP[mood]
  if (!s) return
  ensureImpact()
  impactTextEl.textContent = s.t
  impactTextEl.classList.toggle('dshwv-impact-red', !!s.red)
  // 重新触发动画
  impactEl.classList.remove('dshwv-impact-show')
  void impactEl.offsetWidth // 重排以重启动画
  impactEl.classList.add('dshwv-impact-show')
  if (shoutTimer) { clearTimeout(shoutTimer); shoutTimer = null }
...
function endDrag(e, clickAllowed) {
  ...
  if (clickAllowed && !drag.moved) { playActionClip(); showBubble(); refresh(true); return }
  var dx = e.clientX - drag.startX
  var dy = e.clientY - drag.startY
