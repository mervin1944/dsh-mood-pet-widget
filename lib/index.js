...
// driven by action clips; the static whale PNG (<img> + /dsh-whale/image.png)
...
var ACTION_LIST_URL = '/dsh-whale/action/list.json'
var ACTION_DATA_URL = '/dsh-whale/action/data.webm'
...
// 前端持续轮询 /dsh-whale/mood.json，根据 AI 当前操作状态自动切换表情。
...
function apply(ctx) {
    ...
    function listActionClips() {
      ...
      try {
        ...
        actionClipCache = files.map((f) => ({
          name: f,
          url: '/dsh-whale/action/' + encodeURIComponent(f),
        }))
    ...
    disposers.push(ctx.webServer.register({
      kind: 'exact',
      path: '/dsh-whale/action/list.json',
      handler: (req, res) => {
    ...
    disposers.push(ctx.webServer.register({
      kind: 'exact',
      path: '/dsh-whale/action/data.webm',
      handler: (req, res) => {
        try {
